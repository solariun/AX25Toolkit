#!/usr/bin/env bash
# =============================================================================
#  AX25Toolkit — AX.25 BBS Station Installer (Linux)
...
AX25_PORT_DESC="AX25Toolkit VHF Port"       # Port description in axports
...
# ── BBS settings ─────────────────────────────────────────────────────────────
BEACON_TEXT="!0000.00N/00000.00W>AX25Toolkit AX.25 BBS"
...
# ── Installation paths ───────────────────────────────────────────────────────
AX25Toolkit_REPO="https://github.com/solariun/AX25Toolkit.git"
AX25Toolkit_BRANCH="main"
INSTALL_DIR="/opt/ax25toolkit"              # Where source is cloned & configs live
...
banner() {
    echo ""
    echo -e "${BOLD}╔══════════════════════════════════════════════════════════╗${RESET}"
    echo -e "${BOLD}║          AX25Toolkit — AX.25 BBS Station Installer          ║${RESET}"
    echo -e "${BOLD}╚══════════════════════════════════════════════════════════╝${RESET}"
    echo ""
}
...
# =============================================================================
#  STEP 4 — Clone and build AX25Toolkit
# =============================================================================
section "Building AX25Toolkit"

if [[ -d "${INSTALL_DIR}/.git" ]]; then
    info "Repository already exists at ${INSTALL_DIR} — pulling latest"
    cd "${INSTALL_DIR}"
    git fetch origin
    git checkout "${AX25Toolkit_BRANCH}"
    git pull origin "${AX25Toolkit_BRANCH}"
    ...
    info "Cloning ${AX25Toolkit_REPO} → ${INSTALL_DIR}"
    git clone --branch "${AX25Toolkit_BRANCH}" "${AX25Toolkit_REPO}" "${INSTALL_DIR}"
    cd "${INSTALL_DIR}"
fi
...
cat > "${BBS_INI}" <<INIEOF
; =============================================================================
;  AX25Toolkit Configuration — generated by install.sh
;  $(date '+%Y-%m-%d %H:%M:%S')
; =============================================================================
...
if [[ "${BLE_BRIDGE_BUILT}" -eq 1 ]]; then
    ...
    cat > "${SYSTEMD_DIR}/ax25tk-ble-bridge.service" <<SVCEOF
[Unit]
...
svc_status "ax25tk-ble-bridge.service"
svc_status "ax25tk-kissattach.service"
svc_status "ax25tk-bbs.service"
svc_status "ax25tk-linpac.service"
